...
export async function GET(req: NextRequest) {
  ...
  const handler = async (request: NextRequest) => {
    const symbols = (new URL(request.url).searchParams.get("symbols") || "AAPL,MSFT,GOOGL,TSLA,NVDA")
    ...
    return NextResponse.json({ timestamp: new Date().toISOString(), count: results.length, data: results });
  };
